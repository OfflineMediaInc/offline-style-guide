import type { Meta, StoryObj } from "@storybook/react"
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "./card"
import { Button } from "./button"
import { Input } from "./input"
import { Label } from "./label"
import { Badge } from "./badge"
import { Plus } from "lucide-react"

const meta: Meta<typeof Card> = {
  title: "UI/Card",
  component: Card,
  tags: ["autodocs"],
  parameters: {
    docs: {
      description: {
        component: `
## Offline Card Component

Cards use composition for flexible layouts.

### Size Patterns
- **Default**: Standard padding (p-6)
- **Small**: Compact padding (p-4) - use className overrides

### Composition
- **Card**: Container with border and shadow
- **CardHeader**: Top section (add \`border-b\` for separator)
- **CardTitle**: Heading text
- **CardDescription**: Subtitle/helper text
- **CardContent**: Main content area
- **CardFooter**: Actions (add \`border-t\` for separator)

### Offline Shadows
- \`shadow-card\` - subtle layered shadow
- \`shadow-elevated\` - more prominent shadow
        `,
      },
    },
  },
}

export default meta
type Story = StoryObj<typeof meta>

export const DefaultSize: Story = {
  render: () => (
    <Card className="w-[350px]">
      <CardHeader>
        <CardTitle>Default Card</CardTitle>
        <CardDescription>This card uses the default size variant.</CardDescription>
      </CardHeader>
      <CardContent>
        <p className="text-sm">
          The card component supports a size prop that defaults to &quot;default&quot; for standard spacing and sizing.
        </p>
      </CardContent>
      <CardFooter>
        <Button className="w-full">Action</Button>
      </CardFooter>
    </Card>
  ),
}

export const SmallSize: Story = {
  render: () => (
    <Card className="w-[350px]">
      <CardHeader className="p-4 pb-2">
        <CardTitle className="text-lg">Small Card</CardTitle>
        <CardDescription>This card uses the small size variant.</CardDescription>
      </CardHeader>
      <CardContent className="p-4 pt-0">
        <p className="text-sm">
          The card component supports a size prop that can be set to &quot;sm&quot; for a more compact appearance.
        </p>
      </CardContent>
      <CardFooter className="p-4 pt-0">
        <Button className="w-full">Action</Button>
      </CardFooter>
    </Card>
  ),
}

export const HeaderWithBorder: Story = {
  render: () => (
    <Card className="w-[350px]">
      <CardHeader className="border-b">
        <CardTitle>Header with Border</CardTitle>
        <CardDescription>This is a card with a header that has a bottom border.</CardDescription>
      </CardHeader>
      <CardContent className="pt-6">
        <p className="text-sm">
          The header has a border-b class applied, creating a visual separation between the header and content sections.
        </p>
      </CardContent>
    </Card>
  ),
}

export const FooterWithBorder: Story = {
  render: () => (
    <Card className="w-[350px]">
      <CardContent className="pt-6">
        <p className="text-sm">
          The footer has a border-t class applied, creating a visual separation between the content and footer sections.
        </p>
      </CardContent>
      <CardFooter className="border-t pt-6">
        <Button variant="outline" className="w-full">Footer with Border</Button>
      </CardFooter>
    </Card>
  ),
}

export const HeaderWithBorderSmall: Story = {
  name: "Header with Border (Small)",
  render: () => (
    <Card className="w-[350px]">
      <CardHeader className="p-4 pb-3 border-b">
        <CardTitle className="text-lg">Header with Border</CardTitle>
        <CardDescription>This is a small card with a header that has a bottom border.</CardDescription>
      </CardHeader>
      <CardContent className="p-4">
        <p className="text-sm">
          The header has a border-b class applied, creating a visual separation between the header and content sections.
        </p>
      </CardContent>
    </Card>
  ),
}

export const FooterWithBorderSmall: Story = {
  name: "Footer with Border (Small)",
  render: () => (
    <Card className="w-[350px]">
      <CardContent className="p-4">
        <p className="text-sm">
          The footer has a border-t class applied, creating a visual separation between the content and footer sections.
        </p>
      </CardContent>
      <CardFooter className="p-4 border-t">
        <Button variant="outline" className="w-full">Footer with Border</Button>
      </CardFooter>
    </Card>
  ),
}

export const WithImage: Story = {
  render: () => (
    <Card className="w-[350px] overflow-hidden">
      <div className="aspect-video bg-gradient-to-br from-gray-400 to-gray-600" />
      <CardHeader>
        <CardTitle>Beautiful Landscape</CardTitle>
        <CardDescription>A stunning view that captures the essence of natural beauty.</CardDescription>
      </CardHeader>
      <CardFooter>
        <Button className="w-full">
          <Plus className="mr-2 h-4 w-4" />
          Button
        </Button>
      </CardFooter>
    </Card>
  ),
}

export const WithImageSmall: Story = {
  name: "With Image (Small)",
  render: () => (
    <Card className="w-[350px] overflow-hidden">
      <div className="aspect-video bg-gradient-to-br from-gray-400 to-gray-600" />
      <CardHeader className="p-4 pb-2">
        <CardTitle className="text-lg">Beautiful Landscape</CardTitle>
        <CardDescription>A stunning view that captures the essence of natural beauty.</CardDescription>
      </CardHeader>
      <CardFooter className="p-4 pt-0">
        <Button className="w-full">
          <Plus className="mr-2 h-4 w-4" />
          Button
        </Button>
      </CardFooter>
    </Card>
  ),
}

export const Login: Story = {
  render: () => (
    <Card className="w-[350px]">
      <CardHeader>
        <CardTitle>Login to your account</CardTitle>
        <CardDescription>Enter your email below to login to your account</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="email">Email</Label>
          <Input id="email" type="email" placeholder="m@example.com" />
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label htmlFor="password">Password</Label>
            <a href="#" className="text-sm underline">Forgot your password?</a>
          </div>
          <Input id="password" type="password" />
        </div>
      </CardContent>
      <CardFooter className="flex-col space-y-2">
        <Button className="w-full">Login</Button>
        <Button variant="outline" className="w-full">Login with Google</Button>
        <p className="text-sm text-center text-muted-foreground pt-2">
          Don&apos;t have an account? <a href="#" className="underline">Sign up</a>
        </p>
      </CardFooter>
    </Card>
  ),
}

export const MeetingNotes: Story = {
  render: () => (
    <Card className="w-[400px]">
      <CardHeader>
        <div className="flex items-start justify-between">
          <div>
            <CardTitle>Meeting Notes</CardTitle>
            <CardDescription>Transcript from the meeting with the client.</CardDescription>
          </div>
          <Badge variant="outline">Transcribe</Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-sm">
          Client requested dashboard redesign with focus on mobile responsiveness.
        </p>
        <ol className="list-decimal list-inside text-sm space-y-1 text-muted-foreground">
          <li>New analytics widgets for daily/weekly metrics</li>
          <li>Simplified navigation menu</li>
          <li>Dark mode support</li>
          <li>Timeline: 6 weeks</li>
          <li>Follow-up meeting scheduled for next Tuesday</li>
        </ol>
        <div className="flex items-center gap-1">
          <div className="flex -space-x-2">
            <div className="h-8 w-8 rounded-full bg-gray-300 border-2 border-white" />
            <div className="h-8 w-8 rounded-full bg-gray-400 border-2 border-white" />
            <div className="h-8 w-8 rounded-full bg-gray-500 border-2 border-white" />
          </div>
          <span className="text-sm text-muted-foreground ml-1">+8</span>
        </div>
      </CardContent>
    </Card>
  ),
}

export const OfflineShadows: Story = {
  render: () => (
    <div className="flex gap-6">
      <Card className="w-[250px] shadow-card">
        <CardHeader>
          <CardTitle className="text-base">Standard Shadow</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">Uses shadow-card</p>
        </CardContent>
      </Card>
      <Card className="w-[250px] shadow-elevated">
        <CardHeader>
          <CardTitle className="text-base">Elevated Shadow</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">Uses shadow-elevated</p>
        </CardContent>
      </Card>
    </div>
  ),
}

export const PremiumSurface: Story = {
  render: () => (
    <div className="surface-premium w-[300px] p-6">
      <div className="badge-premium mb-4">
        <span>PREMIUM</span>
      </div>
      <h3 className="headline-small mb-2">Premium Card</h3>
      <p className="body-medium text-muted-foreground">
        Features a gold gradient border using the Offline premium styling.
      </p>
    </div>
  ),
}
